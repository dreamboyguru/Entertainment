...
const Movies = ( type ) => {
    ...
    const handleAddBookmark = (value) => {
        ...
        if (!token && !userName) {
            ...
            axios.post('https://entertainmentbackendott.onrender.com/bookmark', { email: userName, video_id: value, type: 'movie' })
            .then(response => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://entertainmentbackendott.onrender.com/videos/${userName}`);
